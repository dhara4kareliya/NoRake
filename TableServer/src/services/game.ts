...
export class GameService {
        ...
        while (reconnectLimits > 0) {
            ...
            try {
                ...
                if (err.code === 'ECONNABORTED') {
                    if (reconnectLimits == 2)
                        this.table.emit('serverdisconn');;
                    continue;
                }
            }
        }
        this.table.emit('reconnectfailed');
        await delay(1000);
        process.exit(1);
    }
    ...
    public async submitSidebet(table: string, token: string, betName: string, betAmount: string, street: string, round_id: number, bigBlind: number, commonCards: string, privateCards: string, isCash: boolean, odd?: number) {
        ...
        while (reconnectLimits > 0) {
            ...
            try {
                ...
                params.append('bet', `${street}_${betName}`);
    ...
    public async submitSideGame(table: string, token: string, betAmount: number, gameType: string) {
        let reconnectLimits = 3;
        while (reconnectLimits > 0) {
            reconnectLimits--;
            try {
                ...
                const res = await this.client.post(url, params);
                console.log(params);
                this.logger.debug(`GameService: Submit Side Game status: ${url}`);
                ...
                this.logger.debug(`GameService: Submit Side Game status: Error: `, err);
                if (err.code === 'ECONNABORTED') {
                    if (reconnectLimits == 2)
                        this.table.emit('serverdisconn');
                    continue;
    ...
    public async submitSideGameResult(table: string, token: string, betId: string, awardAmount: number, round_id: number) {
        let reconnectLimits = 3;
        while (reconnectLimits > 0) {
            reconnectLimits--;
            try {
                ...
                const res = await this.client.post(url, params);
                console.log(params);
                this.logger.debug(`GameService: Submit Side Game Result: ${url}`);
                ...
                this.logger.debug(`GameService: Submit Side Game Result: Error: `, err);
                if (err.code === 'ECONNABORTED') {
                    if (reconnectLimits == 2)
                        this.table.emit('serverdisconn');
                    continue;
